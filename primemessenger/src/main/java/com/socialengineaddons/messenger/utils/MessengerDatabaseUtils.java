...
import com.socialengineaddons.messenger.interfaces.OnCacheCreateListener;
import com.socialengineaddons.messenger.interfaces.OnNewMessageReceivedListener;
...
public class MessengerDatabaseUtils {
    ...
    private int counter, unReadMsgCount = 0;
    ...
    public DatabaseReference getForceUpgradeDatabase () {

        return messengerDatabase.getReference(Constants.FIREBASE_FORCE_UPGRADE);
    }
    ...
    public int getUnReadMessageCount(final String currentUserId) {
        final DatabaseReference chatMembersDb = getChatMembersDatabase();

        getUsersDatabase().child(currentUserId).child("chats").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(final DataSnapshot dataSnapshot) {
                if (dataSnapshot.getChildrenCount() > 0) {
                    ...
                    for (final DataSnapshot chatRoomId : dataSnapshot.getChildren()) {
                        chatMembersDb.child(chatRoomId.getKey()).addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot childDataSnapshot) {
                                ...
                                if(counter == dataSnapshot.getChildrenCount()
                                        && onNewMessageReceivedListener != null) {
                ...
                } else if (onNewMessageReceivedListener != null) {
                    onNewMessageReceivedListener.updateNewMessageCount(0);
                }
            }
